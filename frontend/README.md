...
CREATE TABLE public.contact_connections (
  ...
  connected_contact_id uuid NOT NULL,
  ...
  CONSTRAINT contact_connections_contact_id_fkey FOREIGN KEY (contact_id) REFERENCES public.professional_contacts(id),
  CONSTRAINT contact_connections_connected_contact_id_fkey FOREIGN KEY (connected_contact_id) REFERENCES public.professional_contacts(id)
);
...
CREATE TABLE public.contact_suggestions_tracking (
  id uuid NOT NULL DEFAULT gen_random_uuid(),
  user_id uuid NOT NULL,
  suggested_contact_id uuid NOT NULL,
  action text NOT NULL CHECK (action = ANY (ARRAY['viewed'::text, 'accepted'::text, 'ignored'::text, 'contacted'::text])),
  action_date timestamp without time zone DEFAULT now(),
  notes text,
  CONSTRAINT contact_suggestions_tracking_pkey PRIMARY KEY (id),
  CONSTRAINT contact_suggestions_tracking_user_id_fkey FOREIGN KEY (user_id) REFERENCES public.users(id),
  CONSTRAINT contact_suggestions_tracking_contact_id_fkey FOREIGN KEY (suggested_contact_id) REFERENCES public.professional_contacts(id)
);
...
CREATE TABLE public.mentor_feedback (
  ...
  feedback_type character varying NOT NULL CHECK (feedback_type::text = ANY (ARRAY['general'::character varying, 'resume'::character varying, 'cover_letter'::character varying, 'application'::character varying, 'interview'::character varying, 'strategy'::character varying]::text[])),
  ...
  CONSTRAINT mentor_feedback_relationship_id_fkey FOREIGN KEY (relationship_id) REFERENCES public.mentor_relationships(id),
  CONSTRAINT mentor_feedback_mentor_id_fkey FOREIGN KEY (mentor_id) REFERENCES public.users(id),
  CONSTRAINT mentor_feedback_mentee_id_fkey FOREIGN KEY (mentee_id) REFERENCES public.users(id)
);
CREATE TABLE public.mentor_progress_reports (
  ...
  mentee_notes text,
  created_at timestamp with time zone DEFAULT now(),
  CONSTRAINT mentor_progress_reports_pkey PRIMARY KEY (id),
  CONSTRAINT mentor_progress_reports_mentee_id_fkey FOREIGN KEY (mentee_id) REFERENCES public.users(id),
  CONSTRAINT mentor_progress_reports_relationship_id_fkey FOREIGN KEY (relationship_id) REFERENCES public.mentor_relationships(id)
);
CREATE TABLE public.mentor_relationships (
  id uuid NOT NULL DEFAULT gen_random_uuid(),
  mentee_id uuid NOT NULL,
  mentor_email character varying NOT NULL,
  mentor_id uuid,
  relationship_type character varying NOT NULL CHECK (relationship_type::text = ANY (ARRAY['mentor'::character varying, 'coach'::character varying]::text[])),
  status character varying NOT NULL DEFAULT 'pending'::character varying CHECK (status::text = ANY (ARRAY['pending'::character varying, 'active'::character varying, 'declined'::character varying, 'ended'::character varying]::text[])),
  can_view_profile boolean DEFAULT true,
  can_view_applications boolean DEFAULT true,
  can_view_resumes boolean DEFAULT true,
  can_view_cover_letters boolean DEFAULT true,
  ...
  CONSTRAINT mentor_relationships_mentee_id_fkey FOREIGN KEY (mentee_id) REFERENCES public.users(id),
  CONSTRAINT mentor_relationships_mentor_id_fkey FOREIGN KEY (mentor_id) REFERENCES public.users(id)
);
...
CREATE TABLE public.team_activity (
  ...
  user_id uuid,
  activity_type text NOT NULL CHECK (activity_type = ANY (ARRAY['member_added'::text, 'member_removed'::text, 'role_changed'::text, 'team_created'::text, 'team_updated'::text, 'invitation_sent'::text, 'candidate_added'::text, 'report_generated'::text])),
  activity_data jsonb,
  created_at timestamp with time zone DEFAULT now(),
...
CREATE TABLE public.team_invitations (
  ...
  invitee_id uuid,
  role text NOT NULL CHECK (role = ANY (ARRAY['admin'::text, 'mentor'::text, 'candidate'::text])),
...
CREATE TABLE public.team_members (
  ...
  user_id uuid NOT NULL,
  role text NOT NULL CHECK (role = ANY (ARRAY['admin'::text, 'mentor'::text, 'candidate'::text])),
  permissions jsonb DEFAULT '{"manage_team": false, "invite_members": false, "view_analytics": false, "edit_candidates": false, "view_all_candidates": false}'::jsonb,
  status text DEFAULT 'active'::text CHECK (status = ANY (ARRAY['active'::text, 'inactive'::text, 'removed'::text])),
...
CREATE TABLE public.teams (
  ...
  subscription_type text DEFAULT 'free'::text CHECK (subscription_type = ANY (ARRAY['free'::text, 'basic'::text, 'premium'::text, 'enterprise'::text])),
  subscription_status text DEFAULT 'active'::text CHECK (subscription_status = ANY (ARRAY['active'::text, 'inactive'::text, 'suspended'::text, 'cancelled'::text])),
...
CREATE TABLE public.user_target_companies (
  ...
  CONSTRAINT user_target_companies_user_id_fkey FOREIGN KEY (user_id) REFERENCES public.users(id)
);
