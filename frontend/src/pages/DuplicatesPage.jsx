...
function DuplicatesPage() {
  ...
  const fetchDuplicates = async () => {
    ...
    try {
      ...
      const response = await fetch(import.meta.env.VITE_API_URL + '/duplicates/pending', {
        headers: {
  ...
  const handleMerge = async (masterJobId, duplicateJobIds) => {
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(import.meta.env.VITE_API_URL + '/duplicates/merge', {
        method: 'POST',
        headers: {
  ...
  const handleDismiss = async (duplicateId) => {
    try {
      ...
      const response = await fetch(
        import.meta.env.VITE_API_URL + `/duplicates/dismiss/${duplicateId}`,
        {
          method: 'POST',
          headers: {
            Authorization: `Bearer ${token}`,
