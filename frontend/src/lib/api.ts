...
// ========== User Interview API (UC-079, /interview) ==========
...
export type CreateInterviewPayload = {
  company_name: string;
  interview_date: string; // ISO or "YYYY-MM-DDTHH:mm"
  interview_type: string;
...
export async function listUserInterviews(): Promise<UserInterview[]> {
  const { data } = await api.get('/interview', { withCredentials: true });
  return data;
}
...
export async function createInterview(
  ...
  const { data } = await api.post('/interview/schedule', payload, {
    withCredentials: true,
  });
  return data;
}

export async function getInterviewById(id: string): Promise<UserInterview> {
  const { data } = await api.get(`/interview/${id}`, { withCredentials: true });
  return data;
}

export async function updateUserInterview(
  id: string,
...
export async function deleteUserInterview(
  id: string
